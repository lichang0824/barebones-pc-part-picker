...
function Cooler(props) {
	...
	useEffect(function () {
		const getData = async function () {
			...
			const res = await fetch(`${process.env.REACT_APP_DOMAIN}:${backendPort}/api/parts?part=cooler`);
			const data = await res.json();
			data.map((element) => (element.type = element.water ? 'Water' : 'Air'));
			setData(data);
			setIsLoaded(true);
		}
		getData();
	...
	function select(part_id) {
		const updatePc = async function () {
            ...
            const res = await fetch(`${process.env.REACT_APP_DOMAIN}:${backendPort}/api/pc`, {
                ...
                body: JSON.stringify({pc_id: currentPcID, part: 'cooler', part_id})
            });
            await res.json();
			navigate('/config');
		}
	...
	const columns = [
		...
		{field: 'fans', headerName: 'Fan Count', flex: 1},
		{field: 'fan_size', headerName: 'Fan Size', flex: 1},
	...
	if (!isLoaded) {
		...
		return <div className="Cooler Content" style={{width: props.width}}>
			<h1>Pick Cooler for: {currentPcName}</h1>
			<DataGrid autoHeight
				rows={data}
				columns={columns}
