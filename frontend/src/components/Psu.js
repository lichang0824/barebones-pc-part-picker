...
function Psu(props) {
	...
	useEffect(function () {
		const getData = async function () {
			...
			const res = await fetch(`${process.env.REACT_APP_DOMAIN}:${backendPort}/api/parts?part=psu`);
			const data = await res.json();
			data.map((element) => (element.modular = element.modular ? 'Yes' : 'No'));
			setData(data);
			setIsLoaded(true);
		}
		getData();
	...
	function select(part_id) {
		const updatePc = async function () {
            ...
            const res = await fetch(`${process.env.REACT_APP_DOMAIN}:${backendPort}/api/pc`, {
                ...
                body: JSON.stringify({pc_id: currentPcID, part: 'PSU', part_id})
            });
            await res.json();
			navigate('/config');
		}
	...
	const columns = [
		...
		{field: 'power', headerName: 'Power', type: 'number', flex: 1},
		{field: 'modular', headerName: 'Modular', flex: 1},
	...
	if (!isLoaded) {
		...
		return <div className="Psu Content" style={{width: props.width}}>
			<h1>Pick PSU for: {currentPcName}</h1>
			<DataGrid autoHeight
				rows={data}
				columns={columns}
