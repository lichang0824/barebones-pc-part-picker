...
function Motherboard(props) {
	...
	useEffect(function () {
		const getData = async function () {
			...
			const res = await fetch(`${process.env.REACT_APP_DOMAIN}:${backendPort}/api/parts?part=motherboard`);
	...
	function select(part_id) {
		const updatePc = async function () {
            ...
            const res = await fetch(`${process.env.REACT_APP_DOMAIN}:${backendPort}/api/pc`, {
                ...
                body: JSON.stringify({pc_id: currentPcID, part: 'motherboard', part_id})
            });
            await res.json();
			navigate('/config');
		}
	...
	const columns = [
		...
		{field: 'chipset', headerName: 'Chipset', flex: 1},
		{field: 'memory_type', headerName: 'Memory Type', flex: 1},
		{field: 'PCIe_generation', headerName: 'PCIe', flex: 1},
		{field: 'm_2', headerName: 'm.2 count', type: 'number', flex: 1},
	...
	if (!isLoaded) {
		...
		return <div className="Motherboard Content" style={{width: props.width}}>
			<h1>Pick Motherboard for: {currentPcName}</h1>
			<DataGrid autoHeight
				rows={data}
				columns={columns}
